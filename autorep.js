...
document.addEventListener('keydown', function (e) {
    ...
    if ((e.keyCode == 32) || (e.keyCode == 13)) {
        ...
        s = s.replace(/нагол(?=\s|$)/, 'На обзорных рентгенограммах правого голеностопного сустава в двух проекциях');
        s = s.replace(/накол(?=\s|$)/, 'На обзорных рентгенограммах правого коленного сустава в двух проекциях');
        s = s.replace(/наплеч(?=\s|$)/, 'На обзорных рентгенограммах правого плечевого сустава в двух проекциях');
        s = s.replace(/трав(?=\s|$)/, 'рентген-признаков костных травматических изменений не выявлено');
        s = s.replace(/пер(?=\s|$)/, 'определяется перелом дистального метаэпифиза лучевой кости и отрывной перелом шиловидного отростка локтевой кости');
        s = s.replace(/конс(?=\s|$)/, 'с признаками консолидации');
        s = s.replace(/стад(?=\s|$)/, 'в стадии консолидации');
        s = s.replace(/сос(?=\s|$)/, 'Отмечается обызвествление стенок сосудов.');
        ...
        s = s.replace(/конспер$/, 'Консолидированные переломы ребер справа.');
        ...
        s = s.replace(/рел$/, 'с признаками релаксации');
        s = s.replace(/миома$/, 'Неоднородное обызвествление в проекции малого таза, вероятно, миоматозный узел.');
        s = s.replace(/конт$/, 'Рекомендации: рентген-контроль в динамике через 7-10 дней.');
